...
### Step 6. entity / concept 캐스케이드 추출

> Karpathy 원칙: **"하나의 소스는 10-15개 페이지를 수정할 수 있다"** — entity/concept 캐스케이드는 절대 생략하지 않는다.

1차 페이지(ADR·module·incident 등) 생성 후, 소스 전체를 다시 스캔하여 다음을 추출한다.

**entity 추출 대상** (소스에 등장하는 모든 고유명사):
- 도구 및 프레임워크 (예: FastAPI, Redis, Kafka)
- 팀 및 조직 단위 (예: Platform Team, Data Engineering)
- 사람 (예: 시스템 오너, 의사결정자)
- 외부 회사·서비스 (예: AWS, Stripe)

**concept 추출 대상** (소스에 등장하는 모든 추상 개념):
- 설계 패턴 (예: Circuit Breaker, CQRS)
- 기법·원칙 (예: Event Sourcing, Idempotency)
- 아키텍처 스타일 (예: Hexagonal Architecture)

**처리 규칙**:
- 해당 `entities/<slug>.md` 또는 `concepts/<slug>.md` 파일이 없으면 → 새로 생성
- 이미 존재하면 → 관련 섹션(Related Modules, Related ADRs, Timeline/References)에 1차 페이지 링크 추가
- 1차 페이지의 `related` 필드에도 추출된 entity/concept 페이지 id를 추가 (양방향)
- 소스에서 추출된 entity/concept이 없을 경우에만 이 단계를 생략한다

### Step 7. 영향받는 기존 페이지 식별 및 업데이트
...
10. **entity/concept 캐스케이드 생략 금지**: 소스에 고유명사 개체 또는 추상 개념이 존재하면 Step 6의 entity/concept 추출을 반드시 실행한다. "시간이 없어서" 또는 "1차 페이지만 만들면 충분해서" 생략하지 않는다.
...
- [ ] entity/concept 추출 완료 — 소스에서 고유명사·추상 개념 스캔 후 페이지 생성/업데이트됨 (Step 6)
